...
import generation.files.UIThreadClientCommandDispatcher.UiThreadExecutor
...
class KotlinClientApi(
    ...
    fun sendHelloFromDev(message: HelloFromDev): Boolean =
    ...
    interface OnStartGameListener {
        fun onStartGameReceived(message: StartGame?)
    }
    ...
    private fun notifyStartGameReceived(message: StartGame) {
    ...
    override fun parseMessage(commandId: Int, inputStream: CodedInputStream): MessageLite? {
        return when (commandId) {
            3 -> StartGame.parser().parsePartialFrom(inputStream)
    ...
    override fun dispatchMessage(commandId: Int, message: MessageLite) {
        when (commandId) {
            3 -> notifyStartGameReceived(message as StartGame)
